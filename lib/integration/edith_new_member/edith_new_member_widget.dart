...
import 'edith_new_member_model.dart';
export 'edith_new_member_model.dart';

class EdithNewMemberWidget extends StatefulWidget {
  const EdithNewMemberWidget({
    super.key,
    required this.newcomer,
  });

  final NewcomersRecord? newcomer;

  @override
  State<EdithNewMemberWidget> createState() => _EdithNewMemberWidgetState();
}

class _EdithNewMemberWidgetState extends State<EdithNewMemberWidget> {
  late EdithNewMemberModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EdithNewMemberModel());

    _model.nomTextController ??=
        TextEditingController(text: widget.newcomer?.firstName);
    _model.nomFocusNode ??= FocusNode();

    _model.prenomTextController ??=
        TextEditingController(text: widget.newcomer?.lastName);
    _model.prenomFocusNode ??= FocusNode();

    _model.phoneTextController ??=
        TextEditingController(text: widget.newcomer?.phone);
    _model.phoneFocusNode ??= FocusNode();

    _model.emailTextController ??=
        TextEditingController(text: widget.newcomer?.email);
    _model.emailFocusNode ??= FocusNode();

    _model.adressTextController ??=
        TextEditingController(text: widget.newcomer?.adress);
    _model.adressFocusNode ??= FocusNode();

    _model.visteValue = widget.newcomer!.visit;
    _model.priereSalutValue = widget.newcomer!.salvationPray;
    _model.commentTextController ??=
        TextEditingController(text: widget.newcomer?.commentaire);
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: FlutterFlowDropDown<String>(
                              controller: _model.ageValueController ??=
                                ...
                                _model.ageValue ??= widget.newcomer?.ageGroup,
                              ),
                              options: const [
                                '18 - 24 ans',
                                '25 - 30 ans',
                          ...
                          Padding(
                            ...
                            child: FlutterFlowDropDown<String>(
                              controller: _model.naNcValueController ??=
                                  FormFieldController<String>(
                                _model.naNcValue ??= widget.newcomer?.ncNa,
                              ),
                              options: const ['NA', 'NC'],
                              onChanged: (val) =>
                          ...
                          Padding(
                            ...
                            child: FFButtonWidget(
                              onPressed: () async {
                                await widget.newcomer!.reference.update({
                                  ...createNewcomersRecordData(
                                    ...
                                    visitReason: _model.raisonVisiteValue,
                                    salvationPray: _model.priereSalutValue,
                                    ncNa: _model.naNcValue,
                                    lastEditor: currentUserDocument?.candidate,
                                    commentaire:
